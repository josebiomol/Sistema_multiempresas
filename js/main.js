...
async function initApp() {
  ...
  try {
    ...
    await templates.loadAppTemplates();
...
function showScreen(screenName) {
  ...
  document.getElementById('householdsScreen')?.style.setProperty('display', 'none', 'important');
  document.getElementById('appScreen')?.style.setProperty('display', 'none', 'important');

  const screen = document.getElementById(`${screenName}Screen`);
  if (screen) {
    screen.style.display = screenName === 'authScreen' ? 'flex' : 'flex';
    if (screenName === 'householdsScreen') loadHouseholds();
    if (screenName === 'appScreen') loadItems();
  }
}
